...
_(RBN = tokenized future cashflow / revenue-backed debt instrument)_
...
Treating _future, whether predictable or impredicable cashflows_ as collateral for crypto loans or tokenized bonds for investors (RWA asset) has the potential to become a new DeFi primitive.
We will call it Revenue-Backed Note (RBN).
...
- **Notional / Pendle** – time-based cashflow separation (no default risk)
...
- Direct integration with Uber, iFood, Airbnb, etc. (off-chain attestation required)
- credit scoring
