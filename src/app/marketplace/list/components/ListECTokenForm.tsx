  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isAddress(tokenAddress)) {
        showToast('Invalid token address.', 'info');
        return;
      }
      if (!tokenId.trim() || Number.isNaN(Number(tokenId))) {
        showToast('Invalid token ID.', 'info');
        return;
      }
      if (!price.trim() || Number(price) <= 0) {
        showToast('Invalid price.', 'info');
        return;
      }
      ...
      showToast('Token listed successfully!', 'info');
      setTokenAddress('');
      setTokenId('');
      setPrice('');
      onSuccess();
      ...
      showToast('Failed to list token. Please try again.', 'info');
    } finally {
      setIsListing(false);
    }
  };
